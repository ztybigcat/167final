...
Binding::Binding(std::string objFilename, GLfloat pointSize)
	: pointSize(pointSize)
{
	...
	if (objFile.is_open())
	{
		...
		while (std::getline(objFile, line))
		{
			...
			if (label == "f")
			{
				...
				for (int i = 0; i < 3; i++)
				{
					sscanf(fStrings[i].c_str(), "%d//%d", &vec[i], &nvec[i]);
...
void Binding::setModelMatrixM(glm::mat4 M) {
	model = M;
}

void Binding::draw(glm::mat4 M, GLuint program) {
